...
import { ArrowRight, ArrowLeft, MapPin, ShieldCheck, Mail } from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `${loc.blurb} WC exemption filing for contractors, sole proprietors, LLC members, and corporate officers in ${loc.name}. 15-minute consultation.`,
  ...
  return (
    <>
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-center">
              <FadeIn className="lg:col-span-7">
                ...
                <div className="mt-7 flex flex-col sm:flex-row gap-3">
                  ...
                  <a href={`mailto:${SITE.email}`} className="btn-secondary"><Mail className="h-5 w-5" />Email Us</a>
                </div>
              </FadeIn>

              <FadeIn direction="left" className="lg:col-span-5">
                <div className="rounded-t-[10rem] rounded-b-3xl overflow-hidden border-4 border-white shadow-warm-lg">
                  <img src="/images/coverage.jpg" alt={`Contractor working in ${loc.name}`} className="w-full h-[300px] md:h-[360px] object-cover" loading="lazy" />
                </div>
              </FadeIn>
            </div>
          </div>
        ...
        <section className="bg-cream py-16 md:py-20">
          <div className="container-tight">
            <FadeIn className="max-w-2xl mb-10">
              <span className="eyebrow"><span className="h-px w-8 bg-clay" />Exemption filing for {loc.name} contractors</span>
              <h2 className="mt-3 h-section">The right exemption, filed correctly for {loc.name} contractors.</h2>
              <p className="mt-4 lead">From a sole proprietor to a multi-officer corporation, we file the right exemption for your structure and state.</p>
            </FadeIn>
        ...
        <FAQ items={locFaqs} eyebrow={`${loc.name} — FAQ`} title={<>WC exemption questions for <span className="text-clay">{loc.name}</span> contractors</>} background="sand" />

        <CTABand title={`Filing WC exemptions for ${loc.name} contractors since 2005`} description={`State-specific exemption filing for sole proprietors, LLC members, and officers. Request a consultation online and we’ll come back to you.`} />
      </main>
      <Footer />
    </>
  );
